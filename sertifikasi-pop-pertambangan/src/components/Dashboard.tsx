    ...
    if (assessments.length > 0) {
      ...
      if (completedAssess.length > 0) {
        ...
        completedAssess.forEach(a => {
          if (a.rubricScores) {
            kes += a.rubricScores.keselamatan;
            reg += a.rubricScores.regulasi;
            kep += a.rubricScores.keputusan;
            ris += a.rubricScores.risiko;
            lead += a.rubricScores.kepemimpinan;
            com += a.rubricScores.komunikasi;
          }
        });
      ...
      <div id="stats-grid" className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex items-center gap-4 hover:border-amber-500/30 hover:shadow-md transition">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5 font-medium">Predicted Competency</p>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex items-center gap-4 hover:border-amber-500/30 hover:shadow-md transition">
          ...
          <div>
            <div className="text-xs text-slate-500 font-bold uppercase tracking-wide">Exam Limit</div>
            <div className="text-lg font-bold text-slate-900">3 Days Remaining</div>
            <p className="text-xs text-slate-400 mt-0.5 font-medium">Standar Kompetensi KTT</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col justify-between">
          ...
          <div className="bg-amber-50/70 rounded-xl p-3 border border-amber-200/50 text-xs text-amber-900 leading-relaxed flex gap-2.5 items-start">
            ...
            <span>
              <strong>Rekomendasi Strategis POP:</strong> Fokus belajar saat ini dipindahkan pada <strong>Investigasi Kecelakaan & Manajemen Risiko Lereng</strong> untuk mendongkrak profil leadership sertifikasi Anda di hadapan penilai.
            </span>
          </div>
        </div>
